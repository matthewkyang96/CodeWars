...
function revrot(str, sz) {
    ...
    let chunks = []
    let chunkCount = Math.floor(str.length / sz)
    for(let i = 0, offset = 0; i < chunkCount; i++, offset += sz){
      chunks.push(str.slice(offset, offset + sz))
    }
  
    return chunks.map(subStr => {
      let subStrArr = subStr.split('');
      let sum = subStrArr.reduce((accum,curr)=> accum+ +curr, 0);
      if(sum % 2 == 0){ 
        return subStr.split('').reverse().join('')
      } else {
        subStrArr.push(subStrArr.shift())
